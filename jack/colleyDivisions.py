    ...
    if index + 1 < 10438:
        ...
        if row['team'] in teams['team'].tolist() and games.loc[index+1][2] in teams['team'].tolist():
            if games.loc[index+1][0] == row['game_id']:
                if row['team_score'] > row['opponent_team_score']:
                    winners.append(teamConference[row['team']])
                    losers.append(teamConference[games.loc[index+1][2]])
                else:
                    winners.append(teamConference[games.loc[index+1][2]])
                    losers.append(teamConference[row['team']])
...
for _, row in colleyData.iterrows():
    winner = row['winner']
    loser = row['loser']
...
matrixList = []
for conference1 in teams['Conference'].unique():
    row = []
    for conference2 in teams['Conference'].unique():
        if conference1 == conference2:
            # Diagonal element: total games + 2
            row.append(total_games[conference1] + 2)
        else:
            # Off Diagonal elements = -1 * num of games between team i and team j
            row.append(-1 * (pairwise_results[(conference1, conference2)]['wins'] + pairwise_results[(conference1, conference2)]['losses']))
    matrixList.append(row)

bVector = []

for conference1 in teams['Conference'].unique():
    wins = sum(pairwise_results[(conference1, conference2)]['wins'] for conference2 in teams['Conference'].unique())
    losses = sum(pairwise_results[(conference1, conference2)]['losses'] for conference2 in teams['Conference'].unique())
...
for index, norm_value in enumerate(normalized_values):
    rankList.append([teams['Conference'].unique()[index], float(norm_value)])
...
for item in sortedRankList:
    totalRank += item[1]
for item in sortedRankList:
    dictReturner[item[0]] = item[1]/totalRank

def returnDivisionRank():
    return dictReturner